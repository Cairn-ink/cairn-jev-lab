...
(async () => {
  ...
  try {
    ...
    put('study-scope', `${study.model} · ${study.date} · ${study.uniqueCases} cases × ${study.passes} passes`);
    ...
    for (let i = 0; i < study.passes; i++) {
      const row = make('div', 'pass-row'); row.append(make('span', 'pass-name', `Pass ${i + 1}`));
      const bars = make('div', 'pass-bars');
      for (const [p, cls, name] of [[baseline, 'baseline', 'Baseline'], [preview, 'preview', 'Preview']]) {
        const result = p.passes[i]; const track = make('div', 'bar-track');
        track.setAttribute('role', 'img'); track.setAttribute('aria-label', `Pass ${i + 1}, ${name}: ${result.matched} of ${result.labeledEvaluated} match expected decisions`);
    ...
    put('variation-note', `${preview.variedCases.length}/${study.uniqueCases} cases changed preview decisions across passes. Baseline remains the default; preview is experimental.`);
    const chart = byId('latency-chart');
    chart.setAttribute('aria-label', `${study.completed} requests across three passes. Minimum ${study.latency.min}, median ${study.latency.median}, mean ${study.latency.mean}, p95 ${study.latency.p95}, maximum ${study.latency.max} milliseconds.`);
    ...
    put('study-limits', `${s.falseSaves}/${s.labeledEvaluated - s.expectedSaves} false saves observed; ${s.missedSaves}/${s.expectedSaves} intended saves missed. Repeated cases, not a reliability guarantee.`);
  } catch {
