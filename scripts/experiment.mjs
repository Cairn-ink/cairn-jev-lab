...
if (process.argv.length !== 3 || process.argv[2] !== '--live') {
  console.log('Use node --env-file=/path/to/.env scripts/experiment.mjs --live to run the frozen 20-case experiment.');
  process.exit(0);
}
...
const protocol = await readFile(new URL('evidence/holdout-en-v1/PROTOCOL.md', root), 'utf8');
const cases = parseCases(fixture);
const hash = s => createHash('sha256').update(s).digest('hex');
const report = { kind: 'frozen-synthetic-english-follow-up', startedAt: new Date().toISOString(),
  ...
  plannedCalls: cases.length, attemptedCalls: 0, completed: false, results: [] };
const dir = new URL(`runs/followup-${Date.now()}/`, root);
...
for (const item of cases) {
  ...
  try {
    ...
    console.log(`${item.id}: ${decisions[policy.version].decision} -> ${decisions[previewPolicy.version].decision} (expected ${item.expected})`);
  } catch (error) {
    const code = /^(invalid_answers|network_or_timeout|invalid_provider_response|provider_http_\d{3})$/.test(error.message)
...
report.completed = report.results.length === cases.length && report.results.every(r => !r.error);
await save();
